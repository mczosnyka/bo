...
class Project:
    ...
    ascii_picture: str
        an ASCII representation of the project graph
    ...
    def from_file(path: str) -> Project:
        ...
        with open(path, encoding="utf8") as f:
            contents = [l.rstrip() for l in f.readlines() if len(l.strip()) > 0]

            try:
                ascii_sep_idx = contents.index(ascii_sep)
                ascii_picture = "\n".join(contents[ascii_sep_idx+1:])
                contents = contents[:ascii_sep_idx]
            except ValueError:
                pass

            for task_row in contents:
        ...
        project = Project(name, tasks, ascii_picture)
