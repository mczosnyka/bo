...
class NormalizedAssignmentProblem:
    ...
    def from_problem(problem: AssignmentProblem) -> NormalizedAssignmentProblem:
        ...
        size = max(problem.n_tasks(), problem.n_workers())
        costs = np.zeros((size, size))
        max_cost = np.max(problem.costs)
        for i in range(len(problem.costs)):
            for j in range(len(problem.costs[0])):
                if problem.objective_is_min:
                    costs[i][j] = problem.costs[i][j]
                else:
                    costs[i][j] = (max_cost - problem.costs[i][j])
